        ...
        const url = await axios.get(`https://www.instagram.com/${widgetSettings.id}`)
            ...
            .then(userData => {
                ...
                if ((widgetSettings.showHeader) == 'yes') {
                    widgetElements.header.classList.add('instagram__widget__header');
                    container.appendChild(widgetElements.header);
                    widgetElements.avatar.setAttribute('src', userData.profile_pic_url);
                    widgetElements.avatar.setAttribute('class', 'instagram__widget__header__avatar');
                    widgetElements.header.appendChild(widgetElements.avatar);
                    widgetElements.username.setAttribute('class', 'instagram__widget__header__username');
                    widgetElements.username.textContent += userData.full_name;
                    widgetElements.header.appendChild(widgetElements.username);
                    widgetElements.logo.classList.add("instagram__widget__header__logo")
                    ...
                    widgetElements.panel.classList.add("instagram__widget__panel");
                    widgetElements.panel.innerHTML =
                        `<p class=instagram__widget__panel__stats>${nFormat(userData.edge_owner_to_timeline_media.count)}<br /> <span>posts</span>
                         <p class=instagram__widget__panel__stats>${nFormat(userData.edge_followed_by.count)}<br /> <span>followers</span>
                         <p class=instagram__widget__panel__stats>${nFormat(userData.edge_follow.count)}<br /> <span>following</span>`;
                ...
                widgetElements.gallery.classList.add('instagram__widget__gallery');
                ...
                widgetElements.button.setAttribute('class', 'instagram__widget__button');
            ...
            .then(photos => {
                photos.forEach(photo => {
                    ...
                    div.setAttribute('class', 'instagram__widget__gallery__caption');
